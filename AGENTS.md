...
- `brand/`: fuentes de generación de la marca (`favicon-source.png`, `social-preview-source.png`). Quedan fuera de `public/` a propósito: se versionan pero no se publican.
- `scripts/generate-brand-assets.mjs`: regenera los favicons PNG/ICO desde `brand/favicon-source.png`; `RobotMark.astro` (nav y footer) renderiza el PNG resultante, no queda ningún SVG vectorial del robot en el shell.
