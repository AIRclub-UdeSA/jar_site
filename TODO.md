...
- [x] **Favicon con el ROSMASTER** — el nav y el footer usan el robot pixel-art transparente de `public/favicon-source.png`; el tab usa una micro-marca circular de alto contraste basada en `public/tab-icon-v2.svg`. `npm run brand:icons` genera los PNG e ICO y sus nombres propios invalidan la caché persistente de favicons.
...
- [x] **Página 404** — `src/pages/404.astro` con ilustración pixel-art del robot buscando el recorrido (`public/media/404/robot-perdido.webp`) y accesos a inicio, setup y workshops; GitHub Pages la sirve automáticamente desde el build.
